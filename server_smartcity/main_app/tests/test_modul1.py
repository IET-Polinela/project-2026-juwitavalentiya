...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_02_login_warga_dengan_password_salah(self):
        ...
        # Payload dengan password yang SALAH
        payload = {
            'username': 'warga_test',
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        HASIL YANG DIHARAPKAN:
            ...
            respons berupa HTTP 302 Found dan diarahkan ke halaman login.
        """
        # 1. Arrange: Lakukan simulasi login session sebagai warga biasa
        self.client.login(username='warga_test', password='Password123!')

        # 2. Act: Mencoba mengakses halaman internal admin (report list)
        url = reverse('report_list') 
        response = self.client.get(url)

        # 3. Assert: Memverifikasi sistem memberikan respons pengalihan (HTTP 302)
        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Warga biasa seharusnya ditolak dan di-redirect (HTTP 302) saat mencoba mengakses halaman admin"
        )
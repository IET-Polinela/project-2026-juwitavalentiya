...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Mengakses detail draf milik Warga B secara langsung menggunakan ID-nya
        url = f'/api/report/{self.draft_milik_b.id}/'
        response = self.client.get(url)

        # Verifikasi: Sistem harus menyembunyikan draf orang lain (404 Not Found)
        self.assertEqual(
            response.status_code, 
            status.HTTP_404_NOT_FOUND, 
            "Warga A seharusnya mendapatkan 404 ketika mencoba melihat detail draf milik Warga B"
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # Payload data perubahan yang dikirim oleh Warga A
        payload = {
            'title': 'Judul Diubah Secara Ilegal',
            'category': 'Infrastruktur',
            'description': 'Mencoba memodifikasi draf orang lain.',
            'location': 'Lokasi Diubah',
            'status': 'DRAFT'
        }

        # Mengirim request PUT ke draf milik Warga B
        url = f'/api/report/{self.draft_milik_b.id}/'
        response = self.client.put(url, payload, format='json')

        # Verifikasi: Sistem harus menolak dan mengembalikan 404 Not Found
        self.assertEqual(
            response.status_code, 
            status.HTTP_404_NOT_FOUND, 
            "Warga A seharusnya mendapatkan 404 ketika mencoba memodifikasi draf milik Warga B"
        )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'num_records',
            type=int,
            help='Jumlah data yang ingin dibuat'
        )
        parser.add_argument(
            '--users',
            type=int,
            default=1,
            help='Jumlah pengguna dummy yang dibuat sebagai reporter'
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if num_records <= 0:
            self.stdout.write(self.style.ERROR('Jumlah record harus lebih besar dari 0.'))
            return

        users = list(User.objects.all()[:num_users])

        while len(users) < num_users:
            username = f'dummy_user_{len(users) + 1}'
        ...
        self.stdout.write(self.style.SUCCESS(f'Berhasil membuat {num_records} laporan dummy.'))
